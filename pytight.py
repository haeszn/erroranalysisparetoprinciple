...
bg_image = Image.open("PARETO-BASED LINTER.png")
bg_image = bg_image.resize((1200, 800), Image.LANCZOS)  # Resize if needed
bg_photo = tk.PhotoImage(bg_image)

bg_label = ctk.CTkLabel(root, text="", image=bg_photo)
bg_label.place(relwidth=1, relheight=1)  # Make it cover the full window

# Create a frame for buttons (transparent for better overlay)
control_frame = ctk.CTkFrame(root, fg_color="transparent")
control_frame.place(x=30, y=140)  # Adjust position to match the image
...
select_files_button = ctk.CTkButton(
    ...
    border_width=3, 
    border_color='#FFFFFF', 
    command=lambda: threading.Thread(target=run_analysis).start() # Replace with your function
)
select_files_button.pack(pady=5, fill="x")

# Create the Run Analysis button
run_analysis_button = ctk.CTkButton(
        master=control_frame, 
        text="RUN ANALYSIS", 
        font=("Arial", 16, "bold"), 
        fg_color="#E91E63", 
        ...
        border_width=3, 
        border_color='#FFFFFF', 
        command=lambda: threading.Thread(target=lambda: threading.Thread(target=run_analysis).start()))# Replace with your function
           
run_analysis_button.pack(pady=5, fill="x")

# Create a frame for displaying selected files
selected_files_text = ScrolledText(root, wrap="word", font=("Arial", 12), height=5, bg='#212121', fg="white")
selected_files_text.place(x=220, y=140, width=900, height=80)  # Adjust position and size

# Create a frame for the chart
chart_frame = ctk.CTkFrame(root, fg_color="transparent", border_width=2, border_color="white")
chart_frame.place(x=30, y=250, width=550, height=300)

# Create a frame for feedbacks
feedback_frame = ctk.CTkFrame(root, fg_color="transparent", border_width=2, border_color="white")
feedback_frame.place(x=620, y=250, width=550, height=300)
